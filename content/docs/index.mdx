...
<Callout type="warn">
	A minimum version of Node.js 18 required, note that Node.js 23.1 might have
	problems with Next.js production build.
</Callout>

<Tabs items={["npm", "pnpm", "yarn", "bun"]}>
	<Tab value="npm">```bash npm create fumadocs-app ```</Tab>
	<Tab value="pnpm">```bash pnpm create fumadocs-app ```</Tab>
	<Tab value="yarn">```bash yarn create fumadocs-app ```</Tab>
	<Tab value="bun">```bash bun create fumadocs-app ```</Tab>
</Tabs>
...
<Callout title="From Existing Codebase?">
	You can follow the [Manual Installation](/docs/manual-installation) guide to
	get started.
</Callout>
...
<Callout title="How to fix stylings not being applied in Monorepo?">
	Make sure to include the `fumadocs-ui` package in your Tailwind CSS content
	configuration.
</Callout>
