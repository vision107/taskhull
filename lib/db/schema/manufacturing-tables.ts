...
export const buildTable = pgTable(
	"build",
	{
		...
		/**
		 * When set, this build is that member's private list: not a project,
		 * not shared with the team. One per (organization, user).
		 */
	...
	(table) => [
		...
		uniqueIndex("build_org_personal_owner_idx")
